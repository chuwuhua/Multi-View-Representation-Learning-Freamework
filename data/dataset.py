...
class DiDiData(Dataset):
    def __init__(self, source_data, index):
    ...
    def load(self):
        ...
        self.weather = torch.load(os.path.join(self.global_feature, 'weather_batch.pt'))
        self.sparse_feature = torch.load(os.path.join(self.global_feature, 'sparse_batch.pt'))
    ...
    def struct_path(self):
        ...
        """
        print('***** struct path *****')
        self.trip_path = os.path.join(self.data_root, 'trip_embedding')
        ...
        # self.sparse_feature = os.path.join(self.data_root, 'sparse_feature')
    ...
    def __getitem__(self, item):
        ...
        global_feature = cat_global_feature(
            [self.distance[item], self.eta[item], self.slice_id[item], self.week[item], self.weather[item],
             self.driver[item]])
        trip_feature = torch.cat([trip, link_time], dim=-1).float()
        sparse_feature = self.sparse_feature[item]
        label = torch.tensor(self.ata[item]).unsqueeze(1).float()
        return global_feature, trip_feature, sparse_feature, self.lengthes[item], label


def index(l,ratio=0.7):
    data = np.array([i for i in range(l)])
    np.random.shuffle(data)
    return data[0:int(l*ratio)],data[int(l*ratio):]


if __name__ == '__main__':
    data = DiDiData(source_data='/DATA/CGW/graduate/data/trainingData', index=[1, 2, 3])
    print(data[1])
    print(len(data))
